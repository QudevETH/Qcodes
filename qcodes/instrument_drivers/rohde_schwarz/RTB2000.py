...
class ScopeTrace(ArrayParameter):
    ...
    def prepare_trace(self) -> None:
        ...
        self.channel._parent.dataformat('ASC,0')
    ...
    def get_raw(self):
        ...
        if instr.run_mode() == 'RUN Nx SINGLE':
            # Wait until the average counter and status are reset
            time.sleep(1)
            while(not instr.average_complete()):
                time.sleep(0.1)
        ...
        output = np.fromstring(raw_vals, sep=',')
    ...
    def __init__(self, parent: Instrument, name: str, channum: int) -> None:
        ...
        self.add_parameter('coupling',
                           ...
                           docstring=('Selects the connection of the channel'
                                      'signal. DCLimit: passes signal unchanged'
                                      ', ACLimit removes DC offset, '
                                      'GND: all channel data is set to 0V'))
        ...
        # TODO (RB): Check if the offset is subtracted again when exporting
        # files manually
        ...
        self.add_parameter('invert',
                           ...
                           get_cmd='CHANnel{}:POLarity?'.format(channum),
                           set_cmd='CHANnel{}:POLarity {{}}'.format(channum),
...
class RTB2000(VisaInstrument):
    ...
    Rohde-Schwarz RTB2000 series oscilloscopes.
    ...
    def __init__(self, name: str, address: str,
        ...
        self._horisontal_divs = int(float(self.ask('TIMebase:DIVisions?')))
        ...
        # Note (RB): Mistake in manual: Set type to REFR for 
        # single acquisiont
        self.add_parameter('acquisition_type',
                            label='Type of the acquisition mode',
                            get_cmd='ACQuire:TYPE?',
                            set_cmd='ACQuire:TYPE {}',
                            vals=vals.Enum('REFR', 'AVER',
                                          'ENV'))

        self.add_parameter('num_acquisitions',
                           label='Number of acquisitions for averaging',
        ...
        self.add_parameter('completed_acquisitions',
                           label='Number of completed acquisitions for averaging',
                           get_cmd='ACQuire:AVERage:CURRent?',
                           get_parser=int)

        self.add_parameter('average_complete',
                            label='Returns the state of averaging',
        ...
        self.add_parameter('sampling_rate',
                           ...
                           'trace.',
                           unit='Sa/s',
                           get_cmd=self._get_sampling_rate)
    ...
    def _set_timebase_position(self, value):
        ...
        """
        self._make_traces_not_ready()
        self.write('TIMEbase:POSition {}'.format(value))

    def _get_sampling_rate(self):
        """
        Get the sampling rate
        """
        val = self.ask('ACQuire:POINts:ARATe?')
    ...
    def _set_trigger_level(self, value):
        ...
        self.write('TRIGger:A:LEVel{} {}'.format(source, value))
    ...
    def _get_trigger_level(self):
        ...
        val = self.ask('TRIGger:A:LEVel{}?'.format(source))
